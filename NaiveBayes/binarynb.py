...
def removeDuplicates(docs: List[List[str]]) -> List[List[str]]:
    '''Returns a new(replaced) list of documents without duplicate words'''
    return [list(dict.fromkeys(docs[i])) for i in range(len(docs))]

def BinaryNBTrain(training_docs_list: List[List[str]], 
    ...
    p_class = [math.log(c/n_doc) for c in n_class] 
    ...
    # group documents according to class and concatenate them
    class_docs = [[]] * len(n_class)
...
def BinaryNBTest(test_documents: List[List[str]], 
                   prior: List[float], 
...
def main():
    processed_docs = removeDuplicates(sample_docs)
    prior_probabilities, log_likelihood, vocabulary = \
        BinaryNBTrain(processed_docs, sample_labels)
    predictions = BinaryNBTest(test_docs, prior_probabilities, 
